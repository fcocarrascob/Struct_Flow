...
/**
 * El panel de propiedades de un gráfico: lo que se dibuja y cómo.
 *
 * Se monta al lado de la hoja mientras el gráfico está en edición (doble clic).
 * Cada cambio va directo a la región (`onCambiar`), y el debounce de evaluación
 * del canvas redibuja la figura: la vista previa es el propio bloque, no una
 * copia que pudiera verse distinta del papel.
 *
 * No calcula nada. Lo que muestra de la evaluación —el error de la región, el
 * aviso de una serie cortada— sale de `result`, igual que en el bloque.
 */
interface Props {
...
const etiqueta = 'text-[10px] font-medium uppercase tracking-wide text-muted';
const boton =
...
export default function PanelGrafico({ region, result, sugerencias, onCambiar, onListo }: Props) {
  ...
  return (
    <aside className="flex h-full w-80 flex-col border-l border-border bg-surface/95 backdrop-blur" aria-label="Propiedades del gráfico">
      <div className="flex items-center justify-between border-b border-border px-2.5 py-2">
        <h2 className="text-xs font-semibold text-ink">Gráfico</h2>
